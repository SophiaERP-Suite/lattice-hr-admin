...
import {
  ...
  MoreVertical, CheckCheck, X, CalendarDays, Timer, DollarSign, User,
  CalendarCheck,
  ClipboardList,
  ChevronRight
} from "lucide-react";
...
import { getEmployeeTimesheets, reviewTimesheet } from "../../api/TimesheetApi";
...
function EmployeeTimesheets() {
  ...
  const { employeeId, employeeName, clientId } = useParams();
  ...
  const decodedEmployerId = useMemo(() => {
    ...
    const decoded = hashIds.decode(String(clientId));
    return decoded.length > 0 ? Number(decoded[0]) : null;
  }, [clientId]);
  ...
  useEffect(() => {
    if (decodedEmployeeId && decodedEmployerId) {
      fetchEmployeeTimesheets(pagination.page);
    }
  }, [statusFilter, pagination.page, decodedEmployeeId, decodedEmployerId]);

  const fetchEmployeeTimesheets = async (page: number) => {
    if (!decodedEmployeeId) return;

    setLoading(true);
    try {
      const response: TimesheetPagedDto = await getEmployeeTimesheets(
        decodedEmployeeId,
        page,
        pagination.pageSize,
      ...
      toast.error("Failed to load timesheets for this employee");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleReview = async (timesheetId: number, action: "approved" | "rejected") => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        await fetchEmployeeTimesheets(pagination.page);
  ...
  // Show message if no employee ID
          ...
          <div className="col-xl-12">
            <div className="page-title-box d-flex-between flex-wrap gap-15">
              <h1 className="page-title fs-18 lh-1 d-flex align-items-center gap-2">
                ...
                Timesheets: {decodedEmployeeName || "Employee"}
              </h1>
              <nav aria-label="breadcrumb">
                <ol className="breadcrumb breadcrumb-example1 mb-0">

                  <li className="breadcrumb-item active" aria-current="page">
                    {decodedEmployeeName || "Employee Details"}
                  </li>

                  <ChevronRight size={15} style={{ position: "relative", top: "5px" }} />
                  <li className="breadcrumb-item"><NavLink to={`/ClientMgt/Timesheet/${clientId}`}>General Timesheet</NavLink></li>

                  <ChevronRight size={15} style={{ position: "relative", top: "5px" }} />
                  <li className="breadcrumb-item">
                    <NavLink to={`/ClientMgt/${clientId}`}>Client Mgt</NavLink>
                  </li>
            ...
            <div className="card">
              <div className="card-body">
                <div className="row g-3 align-items-center">
                  ...
                  <div className="col-md-2">
                    <NavLink className="btn btn-success w-100" to={`/ClientMgt/TimeOffRequests/${clientId}`}>
                      <CalendarCheck size={16} className="me-1" /> Time-Off
                    </NavLink>
                  </div>
                  <div className="col-md-2">
                    <NavLink to={`/ClientMgt/Timesheet/${clientId}`} className="btn btn-warning w-100">
                      <ClipboardList size={16} className="me-1" /> All Timesheets
                    </NavLink>
                  </div>
                </div>
              </div>
          ...
          <div className="col-xl-12">
            <div className="card">
              <div className="card-header d-flex justify-content-between align-items-center">
                <h5 className="mb-0">Employee Timesheets</h5>
                <small className="text-muted">
                  Showing {filteredTimesheets.length} of {pagination.totalCount} record{pagination.totalCount !== 1 ? "s" : ""} for {decodedEmployeeName}
                </small>
              </div>
                          ...
                          {filteredTimesheets.length === 0 ? (
                            <tr>
                              <td colSpan={9} className="text-center py-5 text-muted">
                                ...
                                <h6>No timesheets found for this employee</h6>
