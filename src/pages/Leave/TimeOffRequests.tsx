...
import type { LeaveRequest, LeaveRequestsResponse, DepartmentStats } from "../../types/timeoff";
...
function TimeOffRequests() {
  ...
  const employerId = useMemo(() => {
    return params.clientId ? Number(hashIds.decode(params.clientId)[0]) : null;
  }, [params.clientId]);
  ...
  }, [apiStatus, pagination.page]);

  const fetchLeaveRequests = async (page: number) => {
    setLoading(true);
    try {
      const response: LeaveRequestsResponse = await getAllLeaves(
        page,
        pagination.pageSize,
        apiStatus || undefined,
        Number(employerId)
  ...
  const handleStatusUpdate = async (
    ...
    try {
      if (action === "approved") {
        const response = await approveLeave(requestId, Number(employerId));
        console.log("approved", response)
        if (response.status == 200) {
          toast.success("Request approved sucessfully")
        }
      } else {
        const response = await rejectLeave(requestId, Number(employerId));
        ...
        <div className="row">
          <div className="col-xl-12">
            <div className="page-title-box d-flex-between flex-wrap gap-15">
              <h1 className="page-title fs-18 lh-1">Time-off Management</h1>
              <nav aria-label="breadcrumb">
                <ol className="breadcrumb breadcrumb-example1 mb-0">
                  <li className="breadcrumb-item active">Time-Off Requests</li>
            ...
            <div className="card">
              <div className="card-body">
                <div className="row g-3 align-items-center">
                  ...
                  <div className="col-md-2">
                    <NavLink className="btn btn-success w-100" to={`/ClientMgt/Timesheet/${params.clientId}`}>
                      <CalendarClock size={16} className="me-1" /> TimeSheet
                    </NavLink>
                  </div>
                </div>
              </div>
                            ...
                            filteredRequests.map((request) => (
                              <tr key={request.leaveRequestId}>
                                ...
                                <td>
                                  ...
                                  {/* <span className="fw-medium"><NavLink to={`ClientMgt/TimeOffRequests/${hashIds.encode(request.jobSeekerId.toString())}/${request.employeeName}/${clientId}`}>{request.employeeName}</NavLink></span> */}
                                </td>
                                <td>
              ...
              <div className="modal-body">
                <div className="d-flex align-items-center gap-3 mb-3">
                  <div className="avatar avatar-md">
                    <img
                      src={`http://localhost:5127/${selectedRequest.employeeAvatar}` || "/default-avatar.png"}
                  ...
                  <div>
                    <h6 className="mb-0">{selectedRequest.employeeName}</h6>
